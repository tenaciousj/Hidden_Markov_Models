...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'bounding_box']
        self.contOrDisc = {'length' : DISCRETE, 'bounding_box' : DISCRETE}
        self.numFVals = {'length' : 2, 'bounding_box' : 2}
    ...
    def featurefy( self, strokes ):
        ...
        bb_strokes = self.boundingBoxOfStrokes(strokes)
        bb_points = [i.boundingBox()[0]/bb_strokes for i in strokes]
        
        
        mean_of_bb = sum(bb_points)/float(len(bb_points))
        print "Mean of bounding box: " + str(mean_of_bb) 
        ...
        for i in range(len(strokes)):    
            ...
            bb = strokes[i].boundingBox()[0]/float(self.boundingBoxOfStrokes(strokes))
            print "Bounding box: " + str(bb)
            print "Mean of bb: " + str(mean_of_bb)
            if bb < mean_of_bb:
                d['bounding_box'] = 0
            else:
                d['bounding_box'] = 1
        ...
        self.featureIndices['bounding_box'] = {0: 0, 1: 1}

        return ret

    def boundingBoxOfStrokes( self, strokes ):
        '''Returns psuedo-area of bounding box of image of strokes'''
        min_x = 100000
        min_y = 100000
        max_x = -100000
        max_y = -100000
        ...
        for s in strokes:
            area, x, y, max_x, max_y = s.boundingBox()
...
class Stroke:
    ...
    def boundingBox ( self ):
        '''Returns area of bounding box of stroke'''
        min_x = 100000
        min_y = 100000
        max_x = -100000
        max_y = -100000
